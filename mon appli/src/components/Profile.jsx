...
const ProfileSection = ({ formData = {} }) => {
  const [profilePhoto, setProfilePhoto] = useState('/default-avatar.jpg');
  ...
  const [equipeSuccess, setEquipeSuccess] = useState(null);
  const [equipeError, setEquipeError] = useState(null);
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        // Initialiser le stage si déjà défini
        if (data.stage) {
          setStage(data.stage);
        }

        // Initialiser les membres d'équipe si déjà existants
  ...
  // Envoi des membres d'équipe au backend
